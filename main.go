...
func main() {
	data1 := createSeries(x1, y1)
	data2 := createSeries(x2, y2)
	data3 := createSeries(x3, y3)
	data4 := createSeries(x4, y4)
	...
	for i := 0; i < iterations; i++ {
		...
		OLS(data4)
		endTime := time.Now()
		processT := endTime.Sub(startTime)
		totalPT += processT

	}
	averagePT := totalPT / time.Duration(iterations)
	fmt.Printf("Average processing time for %d iterations: %v\n", iterations, averagePT)
}
